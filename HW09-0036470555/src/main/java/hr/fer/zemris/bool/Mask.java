...
public class Mask {
	...
	if (first.getSize() != second.getSize()) {
	    throw new IllegalArgumentException(
		    "The sizes of both masks must be equal.");
	}

	MaskValue[] vals = new MaskValue[first.getSize()];
	boolean hasDiff = false;
	for (int i = 0; i < first.getSize(); i++) {
	    if (first.getValue(i) == second.getValue(i)) {
		vals[i] = first.getValue(i);
	    } else if (!hasDiff && first.getValue(i) != MaskValue.DONT_CARE
		    && second.getValue(i) != MaskValue.DONT_CARE) {
		vals[i] = MaskValue.DONT_CARE;
		hasDiff = true;
	    } else {
		return null;
	    }
	...
	// special case - masks are equal: not combineable
	if (!hasDiff) {
	    return null;
	}

	return new Mask(vals);
    }
