...
/**
 * Class MaskedBasedBFrepresents a boolean function which is defined by
 * specifying minterm/maxterm and don't care masks. For example: BooleanFunction
 * bF = new MaskBasedBF(
 * <ul>
 * "f", domain, true, Masks.fromStrings("0x00","000x"),
 * Masks.fromStrings("10x0"));
 * </ul>
 * specifies:
 * <ul>
 * f ( A , B , C , D )=∑ m ( 0,1,4 )+∑ d ( 8,10 )
 * 
 * @author Ivan Pavić
 * 
 */
public class MaskBasedBF implements BooleanFunction {
     ...
     *            flag, if set masks are minterms, maxterms otherwise.
     * @param masks
     *            list of indexes
     * @param dontCareMasks
     *            list of dontCares
    ...
    public MaskBasedBF(final String name, final List<BooleanVariable> domain,
	    final boolean masksAreMinterms, final List<Mask> masks,
	    final List<Mask> dontCareMasks) {
	...
	if (!(Collections.disjoint(indexes, dontCares))) {
	    throw new IllegalArgumentException(
		    "There was an intersection in masks!");
	}
    }
	...
	return AuxiliaryClass.get(indexes, indexes2, dontCares,
		masksAreMinterms, domain);
    }
    ...
    /**
     * Getter for dontCareMasks.
     * 
     * @return the dontCareMasks
     */
    ...
    /**
     * Returns true if masks are products.
     * 
